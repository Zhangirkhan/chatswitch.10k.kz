...
<template>
    <div class="h-full flex flex-col">
        <SectionHeader :title="t('profile.profileSection.title')" />
        ...
        <div class="flex-1 overflow-y-auto wa-scrollbar">
            ...
            <!-- Name -->
            <div class="px-6 py-3">
                <div class="text-xs text-[var(--wa-text-secondary)] mb-2">{{ t('profile.profileSection.name') }}</div>
                <div class="flex items-center gap-3 border-b pb-2" :style="{ borderColor: editingName ? 'var(--wa-accent)' : 'var(--wa-border-strong)' }">
                    ...
                    <button
                        ...
                        :title="t('profile.profileSection.edit')"
                    >
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </button>
                    <span v-if="form.processing" class="text-xs text-[var(--wa-text-secondary)]">{{ t('profile.profileSection.saving') }}</span>
                </div>
            ...
            <div class="px-6 py-3 mt-4">
                <div class="text-xs text-[var(--wa-text-secondary)] mb-2">{{ t('profile.profileSection.phone') }}</div>
                <div class="flex items-center gap-3 border-b pb-2" :style="{ borderColor: editingPhone ? 'var(--wa-accent)' : 'var(--wa-border-strong)' }">
                    ...
                    <div v-else class="flex-1 text-[15px] text-[var(--wa-text)] truncate">
                        {{ form.phone || t('profile.profileSection.notSet') }}
                    </div>
                    <div class="flex items-center gap-1 shrink-0">
                        ...
                        <button
                            ...
                            :title="copiedPhone ? t('profile.profileSection.copied') : t('profile.profileSection.copy')"
                        >
