...
import type { TeamChatMessageModel } from './Partials/TeamChatMessage.vue';
...
export type TeamRoomPinned = {
    id: number;
    sender_name: string;
    body_preview: string;
};

export type TeamMsg = TeamChatMessageModel;

export type TeamParticipant = { id: number; name: string };

export type TeamThreadItem =
    | { kind: 'date'; key: string; label: string }
    | { kind: 'unread'; key: string; label: string }
    | { kind: 'message'; key: string; message: TeamMsg };

export type UseTeamChatThreadProps = {
...
export function useTeamChatThread(props: UseTeamChatThreadProps) {
    const { t } = useI18n();
    ...
    const viewerLastReadMessageId = ref<number | null>(null);
    ...
    const shareModalSource = computed((): { kind: 'team'; messageId: number } | null => {
    ...
    function applyConversationHeader(raw: {
        ...
        const convType = raw.type === 'direct' || raw.type === 'department' ? raw.type : null;
        if (!convType) {
            return;
        }
        headerState.value = {
    ...
    function applyReadMetaFromPayload(rm: TeamReadMetaPayload | undefined): void {
        ...
        const vr = rm?.viewer_last_read_message_id;
        viewerLastReadMessageId.value = typeof vr === 'number' && vr > 0 ? vr : null;
    }
    ...
    async function fetchMessages(beforeId?: number | null) {
        ...
        try {
            ...
            conversationType.value = convType === 'direct' || convType === 'department' ? convType : null;
    ...
    function scrollToBottomFab(): void {
        scrollToBottom();
    ...
    function onThreadScroll(e: Event) {
        ...
        showScrollFab.value = distanceFromBottom > 300;
    }
    ...
    async function scrollToHighlightIfNeeded(): Promise<void> {
        ...
        for (let attempt = 0; attempt < 8; attempt++) {
            ...
            const target = root?.querySelector(`[data-team-msg-id="${hid}"]`);
            if (target instanceof HTMLElement) {
                target.scrollIntoView({ block: 'center', behavior: 'smooth' });
                target.classList.add('ring-2', 'ring-[var(--wa-accent)]', 'rounded-lg');
                window.setTimeout(() => {
                    target.classList.remove('ring-2', 'ring-[var(--wa-accent)]', 'rounded-lg');
                }, 2200);
                return;
            }
    ...
    watch(
        ...
        async (id, prevId) => {
            ...
            viewerLastReadMessageId.value = null;
    ...
    function buildMessagePayload(text: string, clientMessageId: string): FormData | Record<string, unknown> {
        ...
        if (files.length > 0) {
            ...
            for (const mentionId of mentionIds) {
                fd.append('mention_user_ids[]', String(mentionId));
            }
    ...
    function messageDayKey(m: TeamMsg): string {
    ...
    function formatThreadDateLabel(dateStr: string): string {
    ...
    const displayedMessages = computed((): TeamMsg[] => {
    ...
    const threadItems = computed((): TeamThreadItem[] => {
        const items: TeamThreadItem[] = [];
        const list = displayedMessages.value;
        const mine = myUserId();
        const viewerLast = viewerLastReadMessageId.value;
        ...
        for (const m of list) {
            ...
            if (dayKey && dayKey !== lastDayKey) {
                const label = m.created_at ? formatThreadDateLabel(m.created_at) : '';
                if (label) {
            ...
            if (
                ...
                items.push({
                    kind: 'unread',
                    key: `unread-${m.id}`,
                    label: t('organization.unreadMessages'),
                });
                unreadInserted = true;
            }
