...
final class CompanyOnboardingService
{
    ...
    private function knowledge(Company $company): void
    {
        $rules = [
            ...
            ['Жалобы и недовольство', 'complaint', 45, 'При жалобе сначала признай неудобство клиента. Уточни номер заказа и суть проблемы. Не спорь и не обещай компенсацию без согласования с менеджером.'],
            ['Возврат и отмена', 'refund', 50, 'Возврат рассматривается по правилам компании. Уточни номер заказа, дату оплаты и причину. Решение подтверждает менеджер — AI не обещает возврат автоматически.'],
            ['Гарантия и брак', 'warranty', 55, 'При браке или дефекте попроси фото и дату получения/монтажа. Дальнейшие шаги — по гарантийной политике компании.'],
            ['AI: конфликтные клиенты', 'ai_guardrail', 60, 'При агрессии, угрозах или требовании возврата не спорь. Дай 1–2 спокойных ответа, затем передай старшему менеджеру и приостанови автоответы.'],
        ];
